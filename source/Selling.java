...
int i = 0;
    public void Update(){
        try{
            ...
            SelectSeller();
            String selectQ = String.format("SELECT * FROM supermarket.seller_bill where bill_id=%s and seller_acc='%s' and prod_name = '%s'", 
            ...
            if(Rs.next()){
                ...
                Add.executeUpdate("update supermarket.seller_bill set quantity="+new_Qty+" where "
                        + "bill_id="+BillId.getText()+" and seller_acc='"+this.account+"'and "
                            + "prod_name='"+ProdName.getText()+"'");
            }
            else{
                Add.executeUpdate("insert into supermarket.seller_bill values("+BillId.getText()+
    ...
    private void BillIdBtnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_BillIdBtnMouseClicked
        if(!BillId.getText().isEmpty())JOptionPane.showMessageDialog(this, "Bill Id Existed");
            ...
            try{
                Con = DriverManager.getConnection("jdbc:mysql://localhost:3306/supermarket","root","NewPassword");
                St = Con.createStatement();
                ResultSet Rs = St.executeQuery("select max(bill_id) from supermarket.seller_bill");
